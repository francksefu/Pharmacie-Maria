...
function dataVente(){
    ...
    //$take = '';
    $sql= ("SELECT * FROM Ventes2, Produit2, Client, DataPersonnel WHERE (Produit2.idProduit = Ventes2.idProduit) and (Client.idClient = Ventes2.idClient and (DataPersonnel.idDataPersonnel = Ventes2.idPersonnel)) group by Operation order by Operation desc");
    ...
    if(mysqli_num_rows($result)>0){
      $valeur = '';
        while($row= mysqli_fetch_assoc($result)){
            //
            $sql1= ("SELECT * FROM Ventes2, Produit2, Client WHERE (Operation = ".$row["Operation"].") and (Produit2.idProduit = Ventes2.idProduit) and (Client.idClient = Ventes2.idClient) order by Operation desc");
